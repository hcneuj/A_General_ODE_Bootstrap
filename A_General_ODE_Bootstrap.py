...
def DE_Generalized(params_trans, ODE_model, init_cond, obs_data, sample_times, unique_times, bounds, n_boot=0, rmsle_return=False, maxiter=10000, tol=1e-5, plot_cost_hist=True, show_usage=False, ODE_names=None):
    ...
    if n_boot == 0:
        ...
        # This line begins the for loop for the bootstrapping
        for b in range(n_boot):

            # These lines create empty lists to store the bootstrapped samples
            obs_b = []
            sampleTime_b = []
            uniqueTime_b = []
            ...
            # These lines create a function to record the cost history of every bootstrap
            cost_hist_b_i = []
            
            def cost_record_boot(xk, convergence):
                boot_cost = CostFunction(xk, params_trans, ODE_model, init_cond,
                                obs_b, sampleTime_b, uniqueTime_b,
                                ODE_names, return_all=False)
                cost_hist_b_i.append(boot_cost)
                return False
            ...
            optimized_b = differential_evolution(
                ...
                maxiter=maxiter,
                tol=tol,
                callback=cost_record_boot)

            cost_hist_boot.append(cost_hist_b_i)

            # This line pulls out the optimized parameters calculated based off of the bootstrapping
            boot_params[b, :] = optimized_b.x
            ...
            # These lines either return both the total and individual RMSLEs from the bootstrap or just the total RMSLE depending on what the user specified
            if rmsle_return:
                Total_RMSLE_b, RMSLEs_b = RMSLE_boot_results
            else:
                Total_RMSLE_b = RMSLE_boot_results
                RMSLEs_b = None
        ...
        if plot_cost_hist:
            ...
            if n_boot > 0:
                plt.figure(figsize=(6,4))
                for i, hist in enumerate(cost_hist_boot):
                    plt.plot(hist, lw=1.5, label=f'Run {i+1}')
                plt.xlabel('Iteration')
                plt.ylabel('Total RMSLE')
                plt.title('Cost History (Bootstraps)')
                plt.grid(False)
                plt.show()
